...
function submitform() {
    ...
    if(plate.value!='' && brand.value!='' && color.value!='') alert("Error!")
    else  car = new Car(plate.value, color.value, brand.value);
...
function addWheels() {
    ...
    if(isWeelvalid(Number(diametro1.value)) && isWeelvalid(Number(diametro2.value)) && isWeelvalid(Number(diametro3.value)) && isWeelvalid(Number(diametro4.value))){
    ...
    } else alert("Error en las ruedas");
...
function isWeelvalid(diametre:number) {
    if(diametre >= 0.4 &&  diametre <=2) return true;
    else return false;
}
